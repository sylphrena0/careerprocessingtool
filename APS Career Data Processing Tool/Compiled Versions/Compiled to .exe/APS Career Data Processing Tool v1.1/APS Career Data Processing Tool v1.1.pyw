...
##  ██   ██ ██      ███████     ██      ██   ██    ██    ███████ ██  ██████ ███████ 
##  
##  APS Career Dara Processing Tool v1.0.pyw -- written by Kirk Kleinsasser
        ...
        if export: #opens export window if selected in results window
            ...
            if name == "CLOSED": #moves past if statement gracefully if user exited filePopup dialog
                ...
                try:
                    ...
                    notify("Fatal Error: File exists and is in use by another program!")
...
def searchAll(keylist,search='fDescription'): #opens window to ask what which conditions to search with, then queries with query()
    ...
    if sector:
        ...
        acadJobs = query(keylist,search, condition=" AND ((Sector LIKE '%academic%') OR (Sector LIKE '%acedemic%'))")
        govJobs = query(keylist,search, condition=" AND Sector LIKE '%government and national lab%'")
        nprofitJobs = query(keylist,search, condition=" AND Sector LIKE '%non-profit%'")
        ...
        tempdict = {'Academic': acadJobs, 'Government/National Lab': govJobs, 'Non-Profit': nprofitJobs, 'Private': privateJobs, 'Other': otherJobs}
    ...
    if level:
        #these are the calls to our query function, and will be columns in our results:
        summJobs = query(keylist,search, condition=" AND JobLevel LIKE '%Summer Research/Internship%'")
        ...
        #make a temp sector result dictionary and add to all results:
        tempdict = {'Summer Research/Internship': summJobs,'Entry Level': entryJobs,'Experienced Level': expJobs}
        data.update(tempdict) #merges the temp result dictionary results with results dictionary
    if degree:
        #these are the calls to our query function, and will be columns in our results:
    ...
    tabledData = pd.DataFrame.from_dict(data) #organizies data into a table (but it's really a dataframe) using pandas. This is easily exportable into GUIs and .csv
